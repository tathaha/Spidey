...
public class PurgeSlashCommand extends SlashCommand {
	public PurgeSlashCommand() {
	...
	public boolean execute(SlashCommandContext ctx) {
		if (!ctx.hasSelfChannelPermissions(ctx.getEvent().getGuildChannel(), getRequiredPermission(), Permission.MESSAGE_HISTORY)) {
			ctx.replyErrorLocalized("commands.purge.messages.failure.no_perms"); // TODO ContextCommandBase#replyNoPerms?
			return false;
		}
	...
	private void respond(SlashCommandContext ctx, User target, long limit) {
